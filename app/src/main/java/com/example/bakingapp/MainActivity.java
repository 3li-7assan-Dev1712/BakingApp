...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<String>, RecipeAdapter.ChooseRecipeInterface{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // use local data for debugging after testing the internet
        recipesNameRecycler.setAdapter(recipeAdapter);
        List<String> names = null;
        try {
            names = JsonUtils.getRecipeNames(JsonRes);
        } catch (Exception e) {
            e.printStackTrace();
        }
        // set the data to the adapter
        recipeAdapter.setRecipesName(names);
        CookRecipeActivity.setJsonRes(JsonRes);
        /*
        using the loader to get the recipes data from the internet
        in a background thread
         */
        // stop the loader and use local data for debugging
